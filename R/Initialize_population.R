Initialize_population<-function(incom, pSize){
  ...
  #initializing data
  population_data[[1]]<-as.data.frame(bcv::impute.svd(incom, k = (nrow(incom)-1),  maxiter=10000)$x)
  population_data[[2]]<-as.data.frame(impute::impute.knn(as.matrix(incom),k = (ncol(incom)-1))$data)
